...
class ArticleGenerator:
  ...
  async def __regenerate_article(self, article: completion_data.CompletionData) -> completion_data.CompletionData:
    ...
    for error in article.errors:
      ...
      if len(new_errors) > 0:
        print(f"[FAILED] Article re-generated with errors for keyword {article.completion_input.keyword}")
      else:
        print(f"Article completion re-generated sucessfuly for keyword {article.completion_input.keyword}")
  ...
  async def generate_article(self, input: completion_data.CompletionInput):
    ...
    if existing_completion is not None:
      print(f"Article with keyword {existing_completion.completion_input.keyword} already exists, skipping\n")
      return
    ...
    self.completion_db.save_completion_data(
      completion_data.CompletionData(
        ...
        raw_content=raw_content if error_or_none(raw_content) is not None else None,
        cleaned_content=get_cleaned_content(raw_content) if error_or_none(raw_content) is not None else None,
        html_content=None,
        meta_desc=metadesc if error_or_none(metadesc) is not None else None,
        meta_title=metatitle if error_or_none(metatitle) is not None else None,
        title=title,
    ...
    if len(errors) > 0:
      print(f"[FAILED] Article generated with errors for keyword {input.keyword}")
    else:
      print(f"Article completion generated sucessfuly for keyword {input.keyword}")
